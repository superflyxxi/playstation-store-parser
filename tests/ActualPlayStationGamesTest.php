...
final class ActualPlayStationGamesTest extends TestCase
{
    ...
    public function testWeirdApostrophe()
    {
        ...
        $this->assertFalse($psGame->hasPsvr(), "PSVR");
    }
    ...
    public function test_PSVR_Support()
    {
        $json = getGameJson("UP1613-CUSA03559_00-THUMPERPS4FULL00");
        $psGame = new PlayStationGame(json_decode($json));

        $this->assertEquals("UP1613-CUSA03559_00-THUMPERPS4FULL00", $psGame->getID(), "ID");
        $this->assertEquals("", $psGame->getURL(), "URL");
        $this->assertEquals("Thumper", $psGame->getDisplayName(), "Display Name");
        $this->assertEquals("PS4", $psGame->getPlatforms()[0], "Platforms[0]");
        $this->assertTrue($psGame->getOriginalPrice() > 0, "Has Original Price");
        $this->assertTrue($psGame->getSalePrice() <= $psGame->getOriginalPrice(), "Sale Price <= Original Price");
        $this->assertNotEquals(0, $psGame->getSalePrice(), "Not equal to 0 for sale price");
        $this->assertFalse($psGame->isEAAccess(), "EA Access Game");
        $this->assertTrue($psGame->hasPsvr(), "PSVR");
    }

    public function test_PSVR_Required()
    {
        $json = getGameJson("UP2613-CUSA09143_00-USSUPERHOTVRGAME");
        $psGame = new PlayStationGame(json_decode($json));

        $this->assertEquals("UP2613-CUSA09143_00-USSUPERHOTVRGAME", $psGame->getID(), "ID");
        $this->assertEquals("", $psGame->getURL(), "URL");
        $this->assertEquals("SUPERHOT VR", $psGame->getDisplayName(), "Display Name");
        $this->assertEquals("PS4", $psGame->getPlatforms()[0], "Platforms[0]");
        $this->assertTrue($psGame->getOriginalPrice() > 0, "Has Original Price");
        $this->assertTrue($psGame->getSalePrice() <= $psGame->getOriginalPrice(), "Sale Price <= Original Price");
        $this->assertNotEquals(0, $psGame->getSalePrice(), "Not equal to 0 for sale price");
